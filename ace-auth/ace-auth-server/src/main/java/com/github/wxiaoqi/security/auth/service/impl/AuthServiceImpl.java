...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    public Object login(JwtAuthenticationRequest authenticationRequest) throws Exception {
        UserInfo info = userService.validate(authenticationRequest);
        if (!StringUtils.isEmpty(info.getId())) {
            Map map = new HashMap<>();
            map.put("accessToken",jwtTokenUtil.generateToken(new JWTInfo(info.getUsername(), info.getId() + "", info.getName())));
